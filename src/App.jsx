...
export default function App() {
  ...
  async function loadFromServer(f) {
    ...
    try {
      ...
      setFileName(`${f.name} · ${data.meta.live ? "live prices" : "server"}${data.meta.xirr != null ? ` · XIRR ${data.meta.xirr}%` : ""}`);
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          {tab === "performance" && (<div className="fade" key="pf">
            ...
            <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}><div className="glass" style={{ padding: "16px 18px" }}><div className="lbl gain" style={{ marginBottom: 8 }}>Top Gainers</div>{analytics.gainers.length ? analytics.gainers.map((m) => moverRow(m, false)) : <div className="mono" style={{ color: "var(--faint)", fontSize: ".78rem", padding: "8px 0" }}>No gainers yet</div>}</div><div className="glass" style={{ padding: "16px 18px" }}><div className="lbl loss" style={{ marginBottom: 8 }}>Top Losers</div>{analytics.losersList.length ? analytics.losersList.map((m) => moverRow(m, true)) : <div className="mono" style={{ color: "var(--gain)", fontSize: ".78rem", padding: "8px 0" }}>No losers — all positions in the green ✓</div>}</div></section>
            <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
